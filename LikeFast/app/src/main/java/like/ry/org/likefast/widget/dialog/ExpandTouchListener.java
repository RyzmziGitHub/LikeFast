...
class ExpandTouchListener implements View.OnTouchListener {
  ...
  @Override public boolean onTouch(View v, MotionEvent event) {
    ...
    if (!(!scrollUp && AppUtil.listIsAtTop(absListView)) && fullScreen) {
      return false;
    }
  ...
  private void onTouchUp(View view, MotionEvent event) {
    ...
    if (!touchUp && params.height < displayHeight && params.height > (displayHeight * 4) / 5) {
      AppUtil.animateContent(contentContainer, displayHeight, new SimpleAnimationListener() {
        @Override
    ...
    if (touchUp && params.height <= defaultContentHeight + 50) {
      AppUtil.animateContent(contentContainer, defaultContentHeight, new SimpleAnimationListener());
      return;
    }
